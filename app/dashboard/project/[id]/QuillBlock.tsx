...
function loadQuill(cb: () => void) {
  ...
  if (!document.getElementById('ql-js')) {
    ...
    s.id = 'ql-js'; s.src = 'https://cdn.jsdelivr.net/npm/quill@2.0.3/dist/quill.js'; s.async = true
...
export function QuillBlock({
  item, onSave, onPin, onArchive, selected, onToggleSelect, isNew,
}: {
  item: Item
  ...
  function schedule(l: string, c: string) {
    if (timer.current) clearTimeout(timer.current)
    timer.current = setTimeout(() => {
      onSave({ label: l || null, content: c || null })
      setSaved(true)
  ...
  useEffect(() => {
    loadQuill(() => {
      ...
      if (item.content) quill.setText(item.content)

      quill.on('text-change', () => {
        schedule(labelRef.current?.value || label, quill.getText().trim())
      })
  ...
  // Border priority: active typing > isNew > selected > default
  const border = active
    ? '2px solid #6366f1'
    : isNew
    ? '2px solid #6366f1'
  ...
  return (
    <div style={{
      background: '#fff', border, borderRadius: 6, height: '100%',
      display: 'flex', flexDirection: 'column', overflow: 'hidden',
      transition: 'border-color 0.15s, box-shadow 0.15s', boxShadow: shadow,
    }}>
      {/* Header */}
      <div style={{
        ...
        {/* Select checkbox */}
        <button onClick={onToggleSelect} title="Select for Oscil AI"
          style={{
        ...
        <input ref={labelRef} type="text" value={label}
          onChange={e => { setLabel(e.target.value); schedule(e.target.value, quillRef.current?.getText().trim() || '') }}
        ...
        <button onClick={() => setShowDel(true)} title="Delete"
          style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '0 1px', flexShrink: 0, color: '#e4e4e7', fontSize: 10, lineHeight: 1, fontFamily: FONT }}>✕</button>
      </div>
      ...
      {/* Quill — toolbar fixed, editor scrolls */}
      <div ref={containerRef} className="block-editor" style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }} />
